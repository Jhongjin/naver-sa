...
- Implemented: membership auth, signup confirmation resend, admin email confirmation fallback, my page session/retry/email-confirmation status, admin user management with user/activity filters, setup planner, keyword/search-query expansion/classification, ad group draft, bid/budget guardrails, approval queue, current-step execution rail, CSV/Excel/Markdown/PDF-ready report exports, dry-run optimization recommendations, Naver execution payload preview, protected test execution route, Naver API readiness layer, admin read-only Naver campaign check, Shopping Search product-group scan, Supabase planning/audit/execution history schema, URL-synced searchable saved history browser, execution draft detail with payload inspection and audit counts, saved draft success summaries, and no-store protected API response policy with `npm run verify`.
