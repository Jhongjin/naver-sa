...
- Public signup now creates an administrator-approval request instead of sending a user email confirmation. Pending users cannot access protected workspace APIs until an admin approves them from member management.
...
- `npm run verify` now runs a runtime error-redaction check that compiles the shared helper and asserts Bearer tokens, query token values, key/value and JSON-shaped secrets, auth headers, cookies, and optional blank text are handled safely.
...
- API policy verification now globally blocks raw auth access echoes, raw env missing arrays, and raw Naver config state in route responses.
...
- Saved-history and public shared-report visible errors now pass through the shared sensitive-text redaction helper before display.
- Saved-history detail now excludes execution idempotency keys from API responses and payload JSON downloads while preserving safe payload inspection fields.
...
- Planning persistence warnings and failure audit events now use the shared sensitive error redaction helper before surfacing Supabase errors.
- Admin user management errors now use the same shared sensitive redaction helper before returning operator-visible failures or audit warnings.
